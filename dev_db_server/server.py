...
dev_db_app = FastAPI(
    title="dev_db: Temporary Dummy Database REST API Server",
    version="1.0.0",
    on_startup=[load_t],
...
async def create_user(identifier: str) -> Union[CreateUserResponse, Response]:
    return Users.create_user(identifier)


@dev_db_app.post("/delete_user", status_code=200)
async def delete_user(user_request: UserRequest) -> Response:
...
async def get_user(user_request: UserRequest) -> Union[GetUserResponse, Response]:
...
@dev_db_app.get("/get_users", status_code=200, include_in_schema=False)
async def get_users(identifier: str) -> Union[GetUsersResponse, Response]:
...
async def set_data(set_data_request: SetDataRequest):
